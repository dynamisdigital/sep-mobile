...
// Entrada da jornada de formalizacao. Lista as propostas APROVADAS do proprio tomador via
// CreditoMobileService — sao as aptas a abrir contrato. NAO existe endpoint global de contratos:
// o contrato so e consultado ao abrir uma proposta (sem N+1). Ownership fica no backend.
@Component({
  selector: 'sep-formalizacao-list',
  standalone: true,
  imports: [IonContent, IonRefresher, IonRefresherContent, IonSpinner, HeaderMobileComponent],
  templateUrl: './formalizacao-list.component.html',
  styleUrl: './formalizacao-list.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class FormalizacaoListComponent implements OnInit {
  ...
  abrirFormalizacao(propostaId: string): void {
    void this.router.navigate(['/app/formalizacao/proposta', propostaId]);
  }
