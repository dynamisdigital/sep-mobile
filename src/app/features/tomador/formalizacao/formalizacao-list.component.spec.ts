...
import { FormalizacaoListComponent } from './formalizacao-list.component';
...
// renderizada ficam no smoke Playwright (M-8.5).
function setup(opts: { listarPropostas?: ReturnType<typeof vi.fn> } = {}) {
  ...
  const component = runInInjectionContext(injector, () => new FormalizacaoListComponent());
  return { component, credito };
}

describe('FormalizacaoListComponent', () => {
  beforeEach(() => TestBed.resetTestingModule());

  it('carrega apenas propostas APROVADAS (aptas a formalizar) sem enviar tomadorId', async () => {
  ...
  it('nao consulta contrato por item da lista (sem N+1)', async () => {
    const { component, credito } = setup({
      listarPropostas: vi
        .fn()
    ...
    await component.carregar();
    // Unica chamada de rede: a listagem. Nenhuma consulta de contrato e disparada por item.
    expect(credito.listarPropostas).toHaveBeenCalledTimes(1);
  });

  it('lista vazia e um estado valido (sem erro)', async () => {
  ...
  it('abrirFormalizacao navega para a rota por proposta', async () => {
    const { component } = setup();
    const router = TestBed.inject(Router);
    const navSpy = vi.spyOn(router, 'navigate').mockResolvedValue(true);
    component.abrirFormalizacao(PROPOSTA_ID);
